...
export default function TextDebateViewer({ fight, onClose }: TextDebateViewerProps) {
  const { fightVotes, toggleFightVote, argumentVotes, toggleArgumentVote, showToast } =
    useAppStore();
      ...
      repliesList: [
        {
          ...
          author: "DevExpert",
          avatar: "🔧",
          text: "Totally agree. I switched 2 years ago.",
          time: "1m ago",
          upvotes: 12,
          downvotes: 1,
        },
        {
          ...
          author: "QuickHacker",
          avatar: "⚡",
          text: "Disagree - too much overhead.",
          time: "30s ago",
          upvotes: 5,
          downvotes: 3,
        },
      ...
      repliesList: [
        {
          ...
          author: "SpeedCoder",
          avatar: "🚀",
          text: "This is the way. Move fast.",
          time: "45s ago",
          upvotes: 28,
          downvotes: 2,
        },
    ...
    <div className="w-full h-full bg-slate-950 flex flex-col">
      ...
      <div className="flex-1 overflow-auto p-4">
        <div className="max-w-3xl mx-auto space-y-3">
          {liveDebates.map((d) => {
            const argVotes = argumentVotes[d.id] || {
              upvotes: 0,
              downvotes: 0,
              userVote: undefined,
            };
            return (
              <div
                key={d.id}
                className={
                  (d.team === "teamA" ? "bg-blue-600/5 " : "bg-red-600/5 ") + "p-3 rounded-lg"
                }
              >
                <div className="flex items-start gap-3">
                  <div className="w-8 h-8 rounded-full flex items-center justify-center bg-slate-800 text-sm">
                    {d.avatar}
                  </div>
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-3 bg-slate-800/40 rounded-lg p-2 w-fit">
                      ...
                      >
                        <ThumbsUp size={12} />{" "}
                        <span className="font-semibold">{argVotes.upvotes}</span>
                      </button>
                      ...
                      >
                        <ThumbsDown size={12} />{" "}
                        <span className="font-semibold">{argVotes.downvotes}</span>
                      </button>
                    </div>
                    ...
                    <div className="flex items-center gap-2 mt-3 text-xs text-slate-400">
                      ...
                      >
                        ...
                        {d.repliesList.length > 0 && (
                          <ChevronDown
                            size={12}
                            className={`transition-transform ${expandedReplies.includes(d.id) ? "rotate-180" : ""}`}
                          />
                        )}
                      </button>
                      <button
                        className="flex items-center gap-1 hover:text-slate-300"
                        onClick={() => setActiveShare(d.id)}
                      >
                        <Share2 size={14} />
                      </button>
                    </div>
                    ...
                    {expandedReplies.includes(d.id) && d.repliesList.length > 0 && (
                      <div className="mt-3 ml-2 pl-3 border-l border-slate-700 space-y-2">
                        {d.repliesList.map((r: any) => {
                          const replyVotes = argumentVotes[r.id] || {
                            upvotes: r.upvotes || 0,
                          ...
                          return (
                            <div key={r.id} className="bg-slate-800/30 p-2 rounded text-xs">
                              <div className="flex items-start gap-2">
                                <div className="w-6 h-6 rounded-full flex items-center justify-center bg-slate-700 flex-shrink-0 text-xs">
                                  {r.avatar}
                                </div>
                                <div className="flex-1 min-w-0">
                                  <div className="flex items-center justify-between">
                        ...
                        {openReplyFor === d.id && (
                          <div className="mt-2 bg-slate-800/40 p-2 rounded">
                            <textarea
                              ...
                              rows={2}
                              className="w-full rounded p-2 bg-slate-800 text-sm text-slate-50 placeholder-slate-500 mb-2"
                            ...
                            <div className="flex justify-end gap-2">
                              <button
                                ...
                                className="px-2 py-1 text-xs rounded text-slate-400 hover:text-slate-300"
                              >
                                Cancel
                              </button>
                              <button
                                onClick={() => {
                                  ...
                                  setLiveDebates((prev) =>
                                    prev.map((dd) =>
                                      dd.id === d.id
                                        ? {
                                            ...dd,
                                            replies: (dd.replies || 0) + 1,
                                            repliesList: [newReply, ...(dd.repliesList || [])],
                                          }
                                        : dd
                                    )
                                  );
                                  setReplyText("");
                                  setOpenReplyFor(null);
                                  showToast("Reply posted", "success", 1400);
                                }}
                                className="px-3 py-1 rounded text-xs bg-gradient-to-r from-blue-600 to-purple-600 text-white font-semibold"
                              >
                                Reply
                              </button>
                            </div>
      ...
      <div className="p-4 border-t border-slate-800 bg-slate-900">
        <div className="max-w-3xl mx-auto flex flex-col gap-3">
          <textarea
            ...
            rows={2}
            className="flex-1 rounded p-3 bg-slate-800 text-slate-50"
          />
          <div className="flex items-center gap-2 relative">
            <div className="relative flex-1">
              <button
                onClick={() => setShowVotePanel((s) => !s)}
                className="p-2 rounded bg-slate-800 text-slate-200 w-full flex items-center justify-center"
              >
            ...
            <button className="px-4 py-2 rounded bg-gradient-to-r from-blue-600 to-purple-600 text-white font-semibold">
              Post
            </button>
          </div>
        </div>
