...
import { Zap, Play, Users, X, Heart, ThumbsDown, Bookmark, MessageCircle, Share2, MoreVertical, Flame, Type, Search } from 'lucide-react';
        ...
        <div className="clean-card relative overflow-hidden rounded-t-xl sm:rounded-lg p-4 sm:p-6 max-w-2xl w-full max-h-[90vh] sm:max-h-[85vh] overflow-y-auto sm:overflow-visible flex flex-col sm:block animate-slideInUp sm:animate-slideIn">
          ...
          <div className="space-y-4 sm:grid sm:grid-cols-2 sm:gap-6">
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-400 mb-2">Fight Type</label>
              ...
              <div className="p-3 rounded-lg bg-slate-800/50 border border-slate-700 text-sm">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className={`font-bold text-xs sm:text-sm ${userEnergy >= 100 ? 'text-green-400' : 'text-red-400'}`}>
                    {userEnergy}
                  </span>
                </div>
              </div>
                ...
                {suggestedOpponents.map((opp, idx) => (
                  ...
                  >
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs bg-slate-700/50 px-2 py-1 rounded text-slate-400 whitespace-nowrap">Online</span>
                    </div>
                  </button>
                ))}
              </div>
...
function WatchFightModal({ isOpen, onClose, fight }: { isOpen: boolean; onClose: () => void; fight: any }) {
...
function JoinFightModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  const availableFights = [
    { id: 101, fighter1: 'TechPro', fighter2: 'CodeMaster', viewers: 234, type: 'visual', status: 'starting' },
    { id: 102, fighter1: 'DesignWizard', fighter2: 'UIEngineer', viewers: 156, type: 'text', status: 'starting' },
    { id: 103, fighter1: 'ReactGuru', fighter2: 'VueExpert', viewers: 89, type: 'text', status: 'starting' },
    { id: 104, fighter1: 'DataNinja', fighter2: 'DatabaseMaster', viewers: 312, type: 'visual', status: 'starting' },
  ];

  const filtered = filterType === 'all' ? availableFights : availableFights.filter(f => f.type === filterType);
    ...
    <>
      ...
      <div className="fixed bottom-0 left-0 right-0 sm:inset-0 z-50 sm:flex sm:items-center sm:justify-center sm:p-4 animate-slideInUp sm:animate-fadeIn">
        <div className="clean-card rounded-t-xl sm:rounded-lg p-4 sm:p-6 max-w-2xl w-full max-h-[90vh] sm:max-h-[85vh] overflow-hidden flex flex-col animate-slideInUp sm:animate-slideIn">
          ...
          <p className="text-xs sm:text-sm text-slate-400 mb-3 sm:mb-4">Accept a challenge and battle live</p>
            ...
            {filtered.map((fight) => (
              ...
              >
                <div className="flex items-center justify-between mb-1.5 sm:mb-2 gap-2">
                  ...
                  <span className="text-xs text-yellow-400 font-bold whitespace-nowrap flex-shrink-0">STARTING</span>
                </div>
...
export default function FightPage() {
  ...
  const handleVote = (fightId: number, team: 'teamA' | 'teamB') => {
    ...
    showToast(`Voted for Team ${team === 'teamA' ? 'A' : 'B'}! 🔥`, 'success', 2000, 'Undo', () => {
      if (prev === undefined) toggleFightVote(fightId, team); else toggleFightVote(fightId, prev);
    });
  };
  ...
  return (
    <div className="animate-fadeIn pb-20">
      ...
      <div className="clean-card p-4 border-b border-slate-700/50">
        ...
        <p className="text-sm text-slate-400">Challenge others or watch epic battles • Thousands of fights happening live!</p>
      </div>
              ...
              >
                      ...
                      <div className="flex gap-2">
                        <button
                          onClick={() => {
                            ...
                            showToast(`You now support ${fight.fighter1}`, 'success', 1800, 'Undo', () => {
                        ...
                        <button
                          onClick={() => {
                            ...
                            showToast(`You now support ${fight.fighter2}`, 'success', 1800, 'Undo', () => {
                ...
                <div className="flex items-center gap-2 mb-4">
                  ...
                  {fight.status === 'starting' && (
                    <div className="inline-flex items-center gap-2 px-3 py-1 rounded-lg bg-yellow-500/20 border border-yellow-500/50">
                      ...
                      <span className="text-yellow-400 text-xs font-bold uppercase">Starting Soon</span>
                    </div>
                  )}
                ...
                <div className="flex items-center justify-between mb-4">
                  <div className="flex-1">
                    <div className={`w-16 h-16 rounded-lg bg-gradient-to-br ${fight.gradient1} mx-auto mb-2 group-hover:scale-110 transition-transform`}></div>
                      <div className="text-center font-bold text-sm text-slate-50 mb-1">{fight.fighter1}</div>
                    ...
                    <div className="text-center text-xs text-slate-400">{fight.fighter1Energy} ⚡</div>
                  </div>
                  ...
                  <div className="flex-1">
                    <div className={`w-16 h-16 rounded-lg bg-gradient-to-br ${fight.gradient2} mx-auto mb-2 group-hover:scale-110 transition-transform`}></div>
                    <div className="text-center font-bold text-sm text-slate-50 mb-1">{fight.fighter2}</div>
                    ...
                    <div className="text-center text-xs text-slate-400">{fight.fighter2Energy} ⚡</div>
                  </div>
                </div>
                ...
                <div className="bg-slate-800/20 rounded-lg p-2 mb-4 text-xs text-slate-400">
                  Support teams from the fight viewer: use the flame button in the chat input to pick a side.
                </div>
        ...
        <div className="clean-card rounded-lg p-6">
          ...
          <div className="space-y-3">
            <div className="flex items-start gap-3">
              <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-purple-600 to-blue-600 flex items-center justify-center text-white font-bold flex-shrink-0 text-sm">1</div>
              <div>
                <div className="font-semibold text-sm text-slate-50 mb-1">Start a Fight</div>
                <div className="text-xs text-slate-400">Use 100 Vynce Energy to challenge someone</div>
              </div>
            </div>
            <div className="flex items-start gap-3">
              <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-purple-600 to-blue-600 flex items-center justify-center text-white font-bold flex-shrink-0 text-sm">2</div>
              <div>
            ...
            <div className="flex items-start gap-3">
              <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-purple-600 to-blue-600 flex items-center justify-center text-white font-bold flex-shrink-0 text-sm">3</div>
              <div>
                <div className="font-semibold text-sm text-slate-50 mb-1">Win Rewards</div>
                <div className="text-xs text-slate-400">Winners earn Energy and exclusive badges</div>
              </div>
            </div>
          </div>
        </div>
      ...
      <FightSearch isOpen={showFightSearch} onClose={() => setShowFightSearch(false)} onSelectFight={handleSearchSelectFight} />
      <WatchFightModal isOpen={showWatchFight} onClose={() => setShowWatchFight(false)} fight={selectedFight} />
