...
export default function CommentsSheet({
  ...
  useEffect(() => {
    ...
    setComments([
      { id: 'c1', author: 'Alex Rivera', avatar: undefined, text: 'Nice take — agreed!', time: '2h' },
      { id: 'c2', author: 'Jordan Lee', avatar: undefined, text: 'Congrats on the launch 🚀', time: '3h' },
    ]);
    // focus
  ...
  return (
    <>
      <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50" onClick={onClose} aria-hidden="true" role="presentation" />

      <div className={`fixed inset-x-0 bottom-0 z-50`}>
        <div className={`max-w-2xl mx-auto px-3 sm:px-4`}>
          <div className={`rounded-3xl bg-slate-800 border border-slate-700 max-h-[85vh] flex flex-col shadow-2xl`}>
            <div className={`flex items-center justify-between p-4 border-b border-slate-700`}>
              <div className="flex items-center gap-3">
                <div className={`w-10 h-10 rounded-full bg-gradient-to-br ${variant === 'drops' ? 'from-amber-500 to-orange-500' : 'from-purple-600 to-blue-600'} flex items-center justify-center shadow-md`} aria-hidden>
                ...
                <div>
                  ...
                  <div className={`text-xs text-slate-400`}>
                    {isFight ? 'Pick a side — results update instantly' : `${commentsCount} replies`}
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2">
                {currentUser && (
                  <div className="text-xs text-right">
                    <div className={`font-semibold text-slate-50`}>{currentUser.displayName || currentUser.name || currentUser.username}</div>
                    <div className={`text-[11px] text-slate-400`}>Active</div>
                  </div>
                )}
                <button onClick={onClose} aria-label="Close" className="p-2 rounded-full hover:bg-slate-700/50 focus:outline-none focus-visible:outline-2 focus-visible:outline-purple-500">
              ...
              {isFight ? (
                <div className="space-y-4">
                  <div className="flex items-center justify-between gap-3">
                    ...
                    >
                      <div className={`w-12 h-12 rounded-full bg-gradient-to-br ${teams[0].color}`} />
                    ...
                    >
                      <div className={`w-12 h-12 rounded-full bg-gradient-to-br ${teams[1].color}`} />
                      <div className="text-left">
                  ...
                  <div className={`text-xs text-slate-400 text-center`}>Voting is anonymous. Refresh to see latest results.</div>
                </div>
              ) : (
                <ul role="list" className="space-y-3">
                  {comments.map((c) => (
                    <li key={c.id} className={`flex gap-3 ${c.id.startsWith('temp-') ? 'opacity-80' : ''}`}>
                      <div className="w-10 h-10 rounded-full bg-slate-700 flex-shrink-0" aria-hidden />
                  ...
                  {comments.length === 0 && (
                    <li className={`text-center text-sm text-slate-400 py-6`}>Be the first to comment</li>
                  )}
                </ul>
              )}
            </div>
            ...
            {!isFight && (
              <div className={`p-3 border-t border-slate-700`}>
                <div className={`flex items-center gap-2 px-3 py-2 rounded-2xl bg-slate-800 border border-slate-700`}>
                  <img className="w-9 h-9 rounded-full object-cover" src={currentUser?.avatar || '/default-avatar.png'} alt={currentUser?.username || 'You'} />
                  <input
                    ref={inputRef}
                    value={input}
