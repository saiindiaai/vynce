...
export const useAppStore = create<AppState>((set) => ({
  ...
  toggleDislike: async (postId: string) => {
    ...
    try {
      const response = await apiToggleDislike(postId);
      // Ensure state matches response
      set((state) => ({
        dislikedPosts: {
          ...state.dislikedPosts,
          [postId]: response.disliked,
        },
      }));
      return response;
    } catch (error) {
  ...
  // Fight Voting
  fightVotes: {},
  toggleFightVote: (fightId: number, team: "teamA" | "teamB") =>
    set((state) => {
  ...
  // Team Messages
  teamMessages: [],
  addTeamMessage: (fightId: number, team: "teamA" | "teamB", message: string) =>
    set((state) => ({
      teamMessages: [
        ...state.teamMessages,
        {
  ...
  // Argument Votes
  argumentVotes: {},
  toggleArgumentVote: (argumentId: number, voteType: "up" | "down") =>
    set((state) => {
  ...
  showToast: (
    ...
    actionLabel?: string,
    action?: () => void
  ) => set({ toast: { id: `toast_${Date.now()}`, message, type, actionLabel, action } }),
